...
This directory contains all control flow related files for the **prober** component.

## Files

### Documentation
- **`CONTROL_FLOWS.md`** - Current probing flow documentation (generated/maintained)
- **`CONTROL_FLOWS_SPEC.md`** - Design-first specification (YAML-based, AI-friendly)

### Tools
- **`analyze_control_flows.py`** - Analyzer and validator for probing flows
...
- **`demo_development_communication.py`** - Demo of probing workflow

## Usage

### For Probing Development
```bash
# Analyze current probing flows
cd control_flows
python analyze_control_flows.py

# Apply planned probing changes
