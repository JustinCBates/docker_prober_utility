# Control Flows for Prober

## Entry Points (How this component gets called)

### CLI Commands (Primary Interface)
- `app.py` ← Called from: Direct execution or container entrypoint
- `collect_host_info.py` ← Called from: CLI or scripts for data collection  
- `view_probe_data.py` ← Called from: CLI for data visualization
- `precheck.sh` ← Called from: deployment scripts for validation
- `deploy.sh` ← Called from: automated deployment workflows

### Programmatic Entry Points  
- `ProbeCollector()` ← Called from: external monitoring systems
- `run_probe_tests()` ← Called from: deploy-manager for validation
- `get_probe_results()` ← Called from: config-manager for health checks
